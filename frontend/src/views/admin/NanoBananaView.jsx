...
const ResultCard = ({ label, imageSrc, accentColor = 'yellow' }) => {
    ...
    return (
        <div className={`group rounded-3xl border overflow-hidden ${colorMap[accentColor]} transition-all duration-300 hover:shadow-xl`}>
            ...
            <img src={imageSrc?.startsWith('http') ? imageSrc : `${BASE_URL}${imageSrc}`} alt={`${label} result`} className="w-full h-64 object-contain bg-black/30 rounded-b-2xl" />
        </div>
    );
};
...
export const NanoBananaView = () => {
    ...
    const canProcess = prompt.trim() && !processing;
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            setProgress(40);
            const response = await fetch(`${BASE_URL}/api/comfy/generate`, {
                method: 'POST',
