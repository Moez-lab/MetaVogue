...
export const HomeView = () => {
    const { setCurrentView, activeProject, createProject, cancelProject, user, orders, updateOrderStatus } = useGlobal();
    ...
    const chartData = useMemo(() => {
        ...
        const completedOrders = orders.filter(o => (o.status === 'Completed' || o.paymentStatus === 'Paid') && o.paymentStatus === 'Paid');
        ...
        <div className="min-h-screen w-full relative bg-white dark:bg-[#050b14] overflow-hidden font-sans text-slate-900 dark:text-white selection:bg-cyan-500/30 transition-colors duration-500">
            ...
            <div className="relative z-10 p-8 max-w-7xl mx-auto space-y-12 pb-20">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    {/* Revenue Trend Graph */}
                    <div className="lg:col-span-3 bg-white/60 dark:bg-[#111827]/60 backdrop-blur-xl rounded-3xl p-6 border border-slate-200 dark:border-white/10 mt-8 transition-colors duration-500 shadow-[0_0_30px_rgba(0,0,0,0.05)] dark:shadow-[0_0_30px_rgba(34,211,238,0.05)]">
                        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
                            <Icon name="BarChart2" className="text-purple-600 dark:text-purple-400" />
                        ...
                        {chartData.some(d => d.revenue > 0) ? (
                            <div className="h-64 flex items-end justify-between gap-2">
                                {chartData.map((data, i) => {
                                    ...
                                    const displayHeight = Math.max(height, data.revenue > 0 ? 5 : 0);
                                    return (
                                        <div key={i} className="flex-1 flex flex-col items-center gap-2 group">
                                            <div className="relative w-full h-full flex items-end">
                                                {data.revenue > 0 ? (
                                                    ...
                                                    <div className="w-full h-1 bg-slate-200 dark:bg-slate-700/50 rounded"></div>
                                                )}
                                            </div>
                                            <span className="text-[10px] font-bold text-slate-500 dark:text-gray-400 uppercase">{data.month}</span>
                                        </div>
                                    );
                                })}
                            </div>
                            ...
                            <div className="h-64 flex flex-col items-center justify-center text-slate-400 dark:text-gray-500">
                                ...
                                <p className="text-lg font-bold pointer-events-none">No Revenue Data Yet</p>
                                <p className="text-sm pointer-events-none">Complete some orders to see your revenue trend</p>
                            </div>
                        )}
                    </div>
