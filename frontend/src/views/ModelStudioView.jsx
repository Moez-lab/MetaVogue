    ...
    return (
        <div className="min-h-screen w-full relative bg-slate-50 dark:bg-[#050b14] overflow-hidden font-sans text-slate-900 dark:text-white selection:bg-cyan-500/30 transition-colors duration-500">
            ...
            <div className="relative z-10 p-8 max-w-7xl mx-auto space-y-8 pb-20">
                ...
                {/* ── Texture Lab Section ── */}
                <div className="mt-12 pt-12 border-t border-slate-200 dark:border-white/10 transition-colors duration-500">
                    <div className="flex items-center gap-3 mb-8">
                        ...
                        <h3 className="text-3xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-slate-900 via-slate-700 to-slate-500 dark:from-white dark:via-gray-200 dark:to-gray-500">
                            Apply Materials
                        </h3>
                    </div>

                    <div className="bg-white/60 dark:bg-[#111827]/60 backdrop-blur-xl p-6 rounded-3xl border border-slate-200 dark:border-white/10 shadow-xl relative overflow-hidden group transition-colors duration-500 max-w-3xl">
                        <div className="absolute -top-20 -right-20 w-40 h-40 bg-purple-500/10 dark:bg-purple-500/20 rounded-full blur-3xl group-hover:bg-purple-500/20 dark:group-hover:bg-purple-500/30 transition-colors duration-500"></div>
                        
                        <div className="relative z-10 space-y-6">
                            <div>
                                <label className="text-xs font-bold uppercase text-purple-600 dark:text-purple-400 tracking-wider flex items-center gap-2 mb-2">
                                    <Icon name="Palette" size={14} /> Texture Prompt
                                </label>
                                <textarea
                                    value={texturePrompt}
                                    onChange={(e) => setTexturePrompt(e.target.value)}
                                    className="w-full h-32 bg-white dark:bg-[#050b14] border border-slate-200 dark:border-white/10 rounded-xl p-4 text-sm text-slate-700 dark:text-gray-300 focus:text-slate-900 dark:focus:text-white focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/50 outline-none resize-none transition-all placeholder-slate-400 dark:placeholder-gray-600"
                                    placeholder="Describe the material (e.g., worn leather, holographic metal, intricate embroidery)..."
                                />
                            </div>
                            ...
                            >
                                {isTexturing ? (
                                    <>
                                        <Icon name="Loader" className="animate-spin" size={20} /> Applying Texture...
                                    </>
                                ) : (
                                    <>
                                        <Icon name="Sparkles" size={20} /> {result ? 'Generate Texture' : 'Generate a Model First'}
                                    </>
                                )}
                            </button>
                        </div>
